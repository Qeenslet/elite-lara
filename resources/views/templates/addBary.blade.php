<h3>Добавление барицентра</h3>
<p>Выделите в форме все звезды, которые имеют общий барицентр, вокруг которого вращаются планеты.</p>
<form id="addBary" action="{{route('addBary')}}" method="POST" class="form-inline">
    <input type="hidden" name="_token" value="{{csrf_token()}}">
    <input type="hidden" name="address" value="{{$addrId}}">
    <label for="stars">Выбор звезд для создания барицентра</label>
    ...
    <div>
        <button type="submit" class="btn btn-warning" id="change_b_star">Добавить барицентр</button>
    </div>
</form>